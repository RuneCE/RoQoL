const Settings = {
    General: {
        ...
        ViewBannedUser: {
            ...
            Description: "Allows you to view a banned user. (Safari users will manually have to put in the url: www.roblox.com/banned-user/USERID)",
            // Supported: async function(){
...
function CreateDiagnoseSection(OptionsList){
    ...
    async function Run(){
        ...
        AuthenticationStatus.innerHTML = `<br><br>Authentication Status: ${Auth.IsAuthed}<br>Current Roblox Account: ${Auth.CurrentUserId}<br>Authenticated Account: ${Auth.UserId}<br>Last Authentication: ${Auth.LastAuthentication ? SecondsToLength(Date.now() / 1000 - Auth.LastAuthentication) + " ago" : "None"}<br>Is Authenticating: ${Auth.IsAuthenticating}<br>Current Authentication Method: ${Auth.CurrentAuthenticationMethod}<br>First Attempt: ${Auth.FirstAttempt}<br>From Storage: ${Auth.FromStorage}<br>Failures in row: ${Auth.AuthenticationFailuresCounter}<br><br>Error: ${SanitizeString(Auth.AuthenticationError)}`
