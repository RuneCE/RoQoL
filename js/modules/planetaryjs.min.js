function CreatePlanetaryJS(_a, _b, window) {
  ...
  var planetaryjs = {
    ...
    planet: function() {
      ...
      planet.projection = d3.geoOrthographic()
        .clipAngle(90);
  ...
  planetaryjs.plugins.pings = function(config) {
    ...
    var drawPing = function(planet, context, now, alive, ping) {
      ...
      var circle = d3.geoCircle().center([ping.lng, ping.lat])
        .radius(alive / ping.options.ttl * ping.options.angle)();
      context.beginPath();
  ...
  planetaryjs.plugins.zoom = function (options) {
    ...
    return function(planet) {
      planet.onInit(function() {
        ...
        const selection = d3.select(planet.canvas)

        if (startScale !== null && startScale !== undefined) {
          zoom.scaleTo(selection, startScale);
        } else {
          zoom.scaleTo(selection, planet.projection.scale());
        }

        zoom
        .clickDistance(99999999)
          .on('start.mapzoom', onZoomStart.bind(planet))
          .on('end.mapzoom', onZoomEnd.bind(planet))
          .on('zoom.mapzoom', function() {
            onZoom.call(planet);
            planet.projection.scale(d3.event.transform.k);
            afterZoom.call(planet);
          })
        d3.select(planet.canvas).call(zoom).on("mousedown.zoom", null)
  ...
  planetaryjs.plugins.drag = function(options) {
    ...
    return function(planet) {
      planet.onInit(function() {
        var drag = d3.drag()
        .clickDistance(99999999)
          .on('start.mapdrag', onDragStart.bind(planet))
          .on('end.mapdrag', onDragEnd.bind(planet))
          .on('drag.mapdrag', function() {
            onDrag.call(planet);
            var dx = d3.event.dx;
            var dy = d3.event.dy;
